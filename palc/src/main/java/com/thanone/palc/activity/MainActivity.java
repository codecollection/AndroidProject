...
public class MainActivity extends Activity {
    ...
    /**
     * 客户在HTML里登录成功后的操作：读取并上传手机参数和位置信息、上传绑定设备id和用户id
     */
                    ...
                    case MESSAGE_WHAT_LOGIN:
                        ...
                        try {
                            ...
                            if (loginInfo != null && loginInfo.length == 3) {
                                ...
                                bindPhoneIdAndUserId(loginInfo[2]);
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
    ...
    private void autoLogin() {
        ...
        if (UtilString.isNotBlank(u) && UtilString.isNotBlank(p)) {
            ...
            webView.post(new Runnable() {
                @Override
                public void run() {
                    webView.loadUrl("javascript:AndroidAutoLogin('" + u + "','" + p + "','" + phoneId + "')");
                }
            });
        }
    }
    ...
    public class MyJavascriptInterface {
        ...
        @JavascriptInterface
        public void login(final String username, final String password, final String id) {
            MyConfig.log("调用Android的login方法" + username + ":" + password + ":" + id);
            mainHandler1.sendMessage(newMessage(MESSAGE_WHAT_LOGIN, new String[]{username, password, id}));
        }
     ...
     * @param userId
     */
    private void bindPhoneIdAndUserId(final String userId) {
        if (UtilString.isNotBlank(userId) && UtilString.isNotBlank(application.getPhoneId())) {
            HttpUtilsHandler.send(HttpUrlUtil.URL_BINDUSERANDPHONE, HttpUrlUtil.url_bindUserAndPhone(application.getPhoneId(), userId));
        }
    }
     ...
     * @param loc        读取位置信息
     * @param showInHtml 是否调用JS的AndroidReadInfoResult方法
     */
