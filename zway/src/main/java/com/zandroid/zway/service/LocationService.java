package com.zandroid.zway.service;
...
import com.zandroid.zway.bean.Location;
...
public class LocationService extends Service {
    ...
    private void saveData(String time, String latitude, String longitude, String address) {
        ...
        try {
            // 保存到本地数据库
            Location loc = new Location(time, latitude, longitude, address, application.getPhoneId());
            ...
            if (UtilAndroid.isNetworkConnected(this)) {
                ...
                if (locationList != null && locationList.size() > 0) {
                    for (Location l : locationList) {
                        ...
                        LbsSaveHelp.poiCreate(l.getLongitude(), l.getLatitude(), l.getAddress(), l.getTime() + "-" + UtilAndroid.getPhoneVersion(), application.getPhoneId(), new HttpCallback() {
                            @Override
                            public void success(String resultJson) {
                                try {
                                    MyConfig.log("上传成功，准备删除本地id为" + id + "的定位数据！");
                                    application.getDbUtils().deleteById(Location.class, id);
                                    MyConfig.log("删除成功，id" + id);
                                } catch (DbException e) {
                                    e.printStackTrace();
                                }
                            }
