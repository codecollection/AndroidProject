...
public class ImageZoomDialog extends Activity implements OnTouchListener,
	...
	private String imgSavePath;// 图片的保存路径

	@Override
	public void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		setContentView(R.layout.zandroid_imagezoomdialog);
		...
		btnSave.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				try {
					...
					UtilAppFile.scanPhoto(ImageZoomDialog.this, filePath);
					Toast.makeText(ImageZoomDialog.this, "保存成功==>"+imgSavePath, Toast.LENGTH_SHORT).show();
				} catch (IOException e) {
					e.printStackTrace();
					Toast.makeText(ImageZoomDialog.this, "保存失败", Toast.LENGTH_SHORT).show();
				}
			}
		});
	...
	private void initData() {
		...
		imgSavePath = getIntent().getStringExtra("img_savePath");
		...
		final Handler handler = new Handler() {
			public void handleMessage(Message msg) {
				if (msg.what == 1 && msg.obj != null) {
					...
					Toast.makeText(ImageZoomDialog.this, ErrMsg, Toast.LENGTH_SHORT).show();
					finish();
				}
			}
		};
			...
			public void run() {
				...
				try {
					...
					if (imgURL.endsWith("portrait.gif")
							|| UtilString.isBlank(imgURL)) {
						bmp = BitmapFactory
								.decodeResource(imgView.getResources(),
										R.drawable.zandroid_imagezoomdialog_close);
					}
					if (bmp == null) {
						// 是否有缓存图片
						...
						if (file.exists()) {
							bmp = UtilAppFile.getBitmap(imgView.getContext(), filename);
						}
					}
					if (bmp == null) {
						...
						if (bmp != null) {
							try {
								// 写图片缓存
								UtilAppFile.saveImage(imgView.getContext(), filename, bmp);
							} catch (IOException e) {
								e.printStackTrace();
							}
							// 缩放图片
							bmp = UtilImage.zoomBitMapForWindow(ImageZoomDialog.this, bmp);
						}
					}
					msg.what = 1;
					msg.obj = bmp;
