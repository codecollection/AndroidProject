package com.thanone.zgps.service;
...
public class LocationService extends Service {
	...
	private static int SCANSPAN_OK = 30 * 1000;// 定位成功后的定位时间间隔：30秒
	
	private static boolean OPEN_GPS = true;// 设置是否打开GPS；使用GPS前提是用户硬件打开GPS；默认是不打开GPS的；如果打开，则会在状态栏一直显示GPS图标。
	private static boolean OPEN_WAKELOCK = true;// 是否开启手机电源锁（保持CPU运转）
	private static boolean UPLOAD_NO_CHANGE = false;// 如果位置没变化是否上传到服务器
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		startLocation();
		return START_STICKY;// START_STICKY：如果service进程被kill掉,随后系统会尝试重新创建service,但是intent会丢失。
	}
	...
	private void setLocationOption(int scanspan) {
		...
		option.setLocationMode(LocationMode.Device_Sensors);// 设置定位模式(Hight_Accuracy[GPS+网络]、Battery_Saving[网络]、Device_Sensors[GPS])
	...
	private void saveData(String time, String latitude, String longitude, String address) {
		...
		try {
			// 保存到本地数据库
			Location loc = new Location(time, latitude, longitude, address, String.valueOf(userId));
			application.getDbUtils().saveBindingId(loc);
			successTime = time;

			// 上传数据，包括以前的数据
			if (UtilAndroid.isNetworkConnected(this)) {
				List<Location> locationList = application.getDbUtils().findAll(Location.class);
				if (locationList != null && locationList.size() > 0) {
					for (Location l : locationList) {
						final Long id = l.getId();
						MyConfig.log("准备上传id为" + id + "的定位数据！");

						HttpUtilsHandler.send(HttpUrlUtil.URL_LOCATION_SAVE,
								HttpUrlUtil.location_save(userId, l.getTime(), l.getLatitude(), l.getLongitude(), l.getAddress()),
								new HttpCallback() {
									@Override
									public void success(String result) {
										try {
											application.getDbUtils().deleteById(Location.class, id);
											MyConfig.log("上传成功，删除本地id为" + id + "的定位数据！");
										} catch (DbException e) {
											e.printStackTrace();
										}
									}
	...
	private void acquireWakeLock() {
		if (null == wakeLock) {
			...
			// SCREEN_DIM_WAKE_LOCK：	保持CPU 运转，保持屏幕显示（有可能是灰的），允许关闭键盘灯
			// SCREEN_BRIGHT_WAKE_LOCK：	保持CPU 运转，保持屏幕高亮，允许关闭键盘灯
			// FULL_WAKE_LOCK：			保持CPU 运转，保持屏幕高亮，键盘灯也保持亮度
